...
if [ ! -f /var/www/sites/default/settings.php ]; then
	...
	cd /var/www/
	drush site-install standard -y --account-name=admin --account-pass=admin --db-url="mysqli://${DATABASE_USER}:${DATABASE_PASSWORD}@${DATABASE_HOST}:${DATABASE_PORT}/${DATABASE_NAME}" --debug
	# killall mysqld
	sleep 10s
fi
supervisord -n
